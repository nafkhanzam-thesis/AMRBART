...
def main():
    ...
    if args.model_name_or_path:
        model = TargetForConditionalGeneration.from_pretrained(
            args.model_name_or_path,
        ...
        model = TargetForConditionalGeneration.from_config(config)
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = TargetForConditionalGeneration.from_pretrained(args.output_dir)
        tokenizer = AMRTokenizer.from_pretrained(args.output_dir)
        model.to(args.device)
